...
static INLINE uint16_t rjpeg_decode_16(const unsigned char *pos)
{
      ...
      for (mbx = 0;  mbx < ctx->mbwidth;  ++mbx)
      {
         for (i = 0, c = ctx->comp;  i < ctx->ncomp;  ++i, ++c)
         {
            for (sby = 0;  sby < c->ssy;  ++sby)
            {
               ...
               {
                  rjpeg_decode_block(ctx, c, &c->pixels[((mby * c->ssy + sby) * c->stride + mbx * c->ssx + sbx) << 3]);
                  if (ctx->error)
                     return;
               }
            }
...
static void rjpeg_upsample_v(struct rjpeg_data *ctx, struct rjpeg_component *c)
{
   ...
   unsigned char *cout = (unsigned char*)malloc((c->width * c->height) << 1);
...
struct rjpeg_data *rjpeg_new(const unsigned char* data, size_t size)
{
